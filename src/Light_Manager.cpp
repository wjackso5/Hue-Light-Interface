...
class Light_Manager {
		...
		bool Light_Manager::getLights(){
			...
			url = "http://" + bridge->ip + ':' + bridge->port + "/api/"+bridge->usernmae+"/lights";
			if(httpC->get(url)){
		...
		bool Light_Manager::getGroups(){
			...
			url = "http://" + bridge->ip + ':' + bridge->port + "/api/"+bridge->usernmae+"/groups";
			if(httpC->get(url)){
				WApplication::instance()->deferRendering();
				return true;
			}
		...
		bool Light_Manager::setLightName(string id,std::string name){
			Wt::Http::Client *httpC = new Wt::Http::Client;
			...
			httpC->done().connect(boost::bind(&Light_Manager::handleHttpResponse,this,_1,_2));
			url="http://" + bridge->ip + ':' + bridge->port + "/api/"+bridge->usernmae+"/lights/"+id;
		...
		bool Light_Manager::setLightState(std::string id,std::string statename,std::string state){
			...
			std::string url;
			Wt::Http::Message *message=new Wt::Http:Message;
			message.setHearder("Content-type","application/Json");
			message->addBodyText(body);
			httpC->done().connect(boost::bind(&Light_Manager::handleHttpResponse,this,_1,_2));
			url="http://" + bridge->ip + ':' + bridge->port + "/api/"+bridge->usernmae+"/lights/"+id+"/state";
			if(httpC->put(url,*message)){
				WApplication::instance()->deferRendering();
				return true;
			}
		...
		void Light_Manager::handleLightResponse(boost::system::error_code err,const Wt::Http::Message& response)
		{
			...
			if (!err && response.status() == 200) {
				...
				for (int i=0;i<size;i++){
					...
					std::string light_name=val.get("name").toString().orIfNull(WString("name:((("));
					Wt::Json::Value states=val.get("state")
					boolean isOn=states.get("on").toBool().orIfNull(false);
